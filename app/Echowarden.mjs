...
async function rpcChainId() {
  ...
  const res = await axios.post(NET.rpcUrl, payload, { timeout: 10_000 });
  return res?.data?.result ?? null;
}

async function getWalletAddresses(walletClient) {
  try {
...
export async function run() {
  ...
  const sdk = new CoinbaseWalletSDK({ appName: "Echowarden", darkMode: false });
  ...
  const targetsDoc = safeReadJson("inputs/targets.json", { targets: [] });
  const targets = Array.isArray(targetsDoc.targets) ? targetsDoc.targets : [];

  const addresses = await getWalletAddresses(walletClient);
